...
function AdminProducts() {
    ...
    const updateImageConfig = (index: number, key: string, value: number) => {
        setCurrentProduct((prev: any) => {
            const newImages = [...(prev?.productImage || [])];
            if (newImages[index]) {
                newImages[index] = { ...newImages[index], [key]: isNaN(value) ? undefined : value };
            }
    ...
    return (
        <div className="flex flex-col gap-8 max-w-[1600px] mx-auto pb-12">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                <Button
                    onClick={() => { setCurrentProduct({ title: '', pricing: 0, categoryName: '', productImage: [], sizes: [], mockup2dFront: '', mockup2dBack: '', printAreaFront: { x: 25, y: 25, width: 50, height: 50 }, printAreaBack: { x: 25, y: 25, width: 50, height: 50 } }); setIsModalOpen(true); }}
                ...
                <div className="flex items-center gap-3 w-full md:w-auto relative">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-3 border rounded-2xl text-sm font-bold transition shadow-sm ${selectedCategory ? 'bg-primary/5 border-primary/20 text-primary' : 'bg-white border-gray-100 text-gray-600 hover:bg-gray-50'
                                }`}
                        >
                                    ...
                                    <div className="max-h-60 overflow-y-auto px-2">
                                        {categories.map((cat: any) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-bold transition-colors mb-1 ${selectedCategory === cat.name ? 'bg-primary text-white' : 'hover:bg-primary/5 text-gray-600'
                                                    }`}
                                            >
                                                {cat.name}
                                            </button>
                                ...
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-30 animate-in fade-in zoom-in-95 duration-200">
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-2.5 text-xs font-bold transition-colors ${sortBy === opt.value ? 'text-primary bg-primary/5' : 'text-gray-600 hover:bg-gray-50'
                                                }`}
                                        >
                                            {opt.label}
                                        </button>
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                                                {currentProduct?.productImage?.map((img: any, i: number) => (
                                                    <div key={i} className="flex flex-col gap-2 bg-gray-50 p-2 rounded-2xl border border-gray-100">
                                                        <div className="relative group aspect-square rounded-xl bg-white border border-gray-50 overflow-hidden shadow-inner translate-z-0">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-1 mt-1">
                                                            <div className="flex items-center gap-1">
                                                                <span className="text-[8px] font-black uppercase text-gray-400 w-3">X</span>
                                                                <Input type="number" placeholder="%" className="h-7 text-[10px] px-2 font-bold" value={img.overlayX ?? ''} onChange={(e) => updateImageConfig(i, 'overlayX', parseInt(e.target.value))} />
                                                            </div>
                                                            <div className="flex items-center gap-1">
                                                                <span className="text-[8px] font-black uppercase text-gray-400 w-3">Y</span>
                                                                <Input type="number" placeholder="%" className="h-7 text-[10px] px-2 font-bold" value={img.overlayY ?? ''} onChange={(e) => updateImageConfig(i, 'overlayY', parseInt(e.target.value))} />
                                                            </div>
                                                            <div className="flex items-center gap-1">
                                                                <span className="text-[8px] font-black uppercase text-gray-400 w-3">W</span>
                                                                <Input type="number" placeholder="%" className="h-7 text-[10px] px-2 font-bold" value={img.overlayWidth ?? ''} onChange={(e) => updateImageConfig(i, 'overlayWidth', parseInt(e.target.value))} />
                                                            </div>
                                                            <div className="flex items-center gap-1">
                                                                <span className="text-[8px] font-black uppercase text-gray-400 w-3">H</span>
                                                                <Input type="number" placeholder="%" className="h-7 text-[10px] px-2 font-bold" value={img.overlayHeight ?? ''} onChange={(e) => updateImageConfig(i, 'overlayHeight', parseInt(e.target.value))} />
                                                            </div>
                                                            {currentProduct?.mockup2dBack && (
                                                                <div className="flex flex-col gap-1 mt-2 w-[180px]">
                                                                    <span className="text-[9px] font-black uppercase text-gray-400">Print Area (Back)</span>
                                                                    <div className="grid grid-cols-2 gap-1">
                                                                        <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">X%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaBack?.x ?? 25} onChange={(e) => setCurrentProduct({...currentProduct, printAreaBack: {...(currentProduct.printAreaBack || {}), x: parseInt(e.target.value)}})} /></div>
                                                                        <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">Y%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaBack?.y ?? 25} onChange={(e) => setCurrentProduct({...currentProduct, printAreaBack: {...(currentProduct.printAreaBack || {}), y: parseInt(e.target.value)}})} /></div>
                                                                        <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">W%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaBack?.width ?? 50} onChange={(e) => setCurrentProduct({...currentProduct, printAreaBack: {...(currentProduct.printAreaBack || {}), width: parseInt(e.target.value)}})} /></div>
                                                                        <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">H%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaBack?.height ?? 50} onChange={(e) => setCurrentProduct({...currentProduct, printAreaBack: {...(currentProduct.printAreaBack || {}), height: parseInt(e.target.value)}})} /></div>
                                                                    </div>
                                                                </div>
                                                            )}
                                                        </div>
                                            ...
                                            <div className="flex gap-6">
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex items-center gap-4 h-[100px]">
                                                        ...
                                                        {currentProduct?.mockup2dFront && (
                                                            <div className="flex flex-col gap-1 mt-2 w-[180px]">
                                                                <span className="text-[9px] font-black uppercase text-gray-400">Print Area (Front)</span>
                                                                <div className="grid grid-cols-2 gap-1">
                                                                    <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">X%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaFront?.x ?? 25} onChange={(e) => setCurrentProduct({...currentProduct, printAreaFront: {...(currentProduct.printAreaFront || {}), x: parseInt(e.target.value)}})} /></div>
                                                                    <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">Y%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaFront?.y ?? 25} onChange={(e) => setCurrentProduct({...currentProduct, printAreaFront: {...(currentProduct.printAreaFront || {}), y: parseInt(e.target.value)}})} /></div>
                                                                    <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">W%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaFront?.width ?? 50} onChange={(e) => setCurrentProduct({...currentProduct, printAreaFront: {...(currentProduct.printAreaFront || {}), width: parseInt(e.target.value)}})} /></div>
                                                                    <div className="flex items-center gap-1"><span className="text-[8px] font-black uppercase text-gray-400 w-3">H%</span><Input type="number" className="h-7 text-[10px] px-2 font-bold" value={currentProduct.printAreaFront?.height ?? 50} onChange={(e) => setCurrentProduct({...currentProduct, printAreaFront: {...(currentProduct.printAreaFront || {}), height: parseInt(e.target.value)}})} /></div>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
